...
public class URDFImporter : MonoBehaviour {
			...
			for(int i=0; i<robotNode.ChildNodes.Count; i++) {
				...
				if(robotNode.ChildNodes[i].Name == "link") {
					...
					for(int j=0; j<linkNode.ChildNodes.Count; j++) {
						if(linkNode.ChildNodes[j].Name == "visual") {
						...
						if(linkNode.ChildNodes[j].Name == "inertial") {
	...
	public Vector3 MeshOrientation = new Vector3(270f, 90f, 0f);
	...
	public class URDFImporter_CE : Editor
	{		
		private URDFImporter Target;

		void Awake() {
			Target = (URDFImporter)target;
		}
		...
		public override void OnInspectorGUI()
		{
			...
			/*
			if(GUILayout.Button("Import Model Variations")) {
				Vector3 defaultOrientation = Target.MeshOrientation;
				...
				for(int i=0; i<4; i++) {
					for(int j=0; j<4; j++) {
						for(int k=0; k<4; k++) {
							index += 1;
							Target.MeshOrientation = new Vector3(i*90f, j*90f, k*90f);
							GameObject actor = Target.Import();
							actor.transform.position = new Vector3(index, 0f, 0f);
						}
					}
				}

				Target.MeshOrientation = defaultOrientation;
			}
			*/
		}
